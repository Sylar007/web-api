...
namespace WebApi.Services
{
    public class EquipmentModelFileService : IEquipmentModelFileService
    {
		...
		public IEnumerable<dynamic> GetFileList(int equipmentModelId)
		{
			try
			{
				return (from ef in _context.equipment_model_file
						join m in _context.media on ef.media_id equals m.id
						where ef.equipment_model_id == equipmentModelId
						select new
						{
		...
		public bool DeleteEquipmentModelFile(int[] equipmentFileIds)
		{
			try
			{
				List<equipment_model_file> list = _context.equipment_model_file.Where((equipment_model_file ef) => equipmentFileIds.Contains(ef.id)).ToList();
				foreach (equipment_model_file equipmentFile in list)
				{
					media medium = _context.media.Where((media m) => m.id == equipmentFile.media_id).FirstOrDefault();
					_context.equipment_model_file.Remove(equipmentFile);
