...
namespace WebApi.Services
{
    public class TaskSubService : ITaskSubService
    {
        ...
        public IList<TaskSubs> GetTaskExecutionSubTree(int woid, int equipment_id, int wo_type_id)
        {
            try
            {
                var query = (from e in _context.equipments
                ...
                foreach (var parent in query)
                {
                    ...
                    foreach (var children in query2)
                    {
                        ...
                        {
                            //mda: cross check if the Task Execution already exist in the Work Order
                            var existTaskSub = GetTaskSubExecutionExisted(woid, equipment_id, wo_type_id, children.task_sub_id).FirstOrDefault();

                            if (existTaskSub != null)
                            {
                                if (!String.IsNullOrEmpty(existTaskSub.Item2))
                                {
                                    subtasksingle.text = children.subtaskName;
                                    subtasksingle.Checked = true;
                                    value subtasksingleValue = new value();
        ...
        public IQueryable<Tuple<int, string>> GetTaskSubExecutionExisted(int woid, int equipment_id, int wo_type_id, int task_sub_id)
        {
            return (from task_sub in _context.task_sub
                    join wo_task_sub in _context.wo_task_sub on task_sub.id equals wo_task_sub.task_sub_id
                    join work_order in _context.work_order on wo_task_sub.wo_id equals work_order.id into woJoin
                    from woj in woJoin.DefaultIfEmpty()
                    where woj.equipment_id == equipment_id && woj.wo_type_id == wo_type_id && wo_task_sub.task_sub_id == task_sub_id && woj.id == woid
                    select new Tuple<int, string>(
                        woj.id,
                        wo_task_sub.wo_task_type
                    ));
        }
